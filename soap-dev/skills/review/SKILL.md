...
1. **判斷是否為誤判** — 有些問題可能是 agent 過度敏感，結合上下文判斷是否真的需要修。**嚴禁以「非本次改動引入」「既有問題」「歷史遺留」等理由排除問題。** 只要問題屬實且確實需要修正，不論是本次改動引入還是原本就存在，一律列入修正清單。「不用修」的唯一合法理由是：問題本身不成立（agent 誤判、理解錯誤、或建議的修正方向錯誤）。
...
驗證時用語必須嚴謹，**不允許「看起來可能是」「應該是」「或許」這類模糊判斷**，主 agent 必須給出明確結論：問題屬實或不屬實、修正方向正確或不正確。排除條目的理由只能是「問題不屬實」或「修正方向錯誤」，不得以「既有問題」「非本次引入」為由排除。
