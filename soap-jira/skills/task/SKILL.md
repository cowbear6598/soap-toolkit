...
```bash
python3 <jira-skill-dir>/scripts/get.py --issue $ARGUMENTS
```
...
```bash
python3 <jira-skill-dir>/scripts/create.py \
  --project <PROJECT_KEY> \
  --summary "<子任務 summary>" \
...
使用 `soap-jira:jira` skill 的 `transition.py` 將 `$ARGUMENTS` 的狀態改為 `In Progress`：

```bash
python3 <jira-skill-dir>/scripts/transition.py --issue $ARGUMENTS --status "In Progress"
```
