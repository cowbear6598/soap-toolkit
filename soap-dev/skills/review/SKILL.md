...
[改動檔案]（本次使用者實際修改，問題嚴重性照常判定）
{changed 清單}

[關聯檔案]（經 import closure 追蹤得到，問題預設嚴重性較低：同類問題請降級為「建議修正」而非「必須修正」，除非屬於安全性或會直接造成 runtime 錯誤）
{related 清單}
...
2. **依照檔案分組調整嚴重性**：
   - 「改動檔案 + 必須修正」→ 保持必須修正
   - 「關聯檔案 + 必須修正」→ 重新判斷：只有屬於安全性或會造成 runtime error 的才維持必須修正，其餘降為建議修正
   - 「改動檔案 / 關聯檔案 + 建議修正」→ 保持建議修正
3. **每個問題只有「要修」和「不用修」** — 不給「可修可不修」「需確認」「視情況而定」這種模稜兩可的選項。如果你無法確定，就自己去讀程式碼搞清楚，然後做出明確判斷。不確定不是一個選項。
