...
### 修正
- plan skill 與 review skill 的 Fallback 規則補上「正規使用路徑反推」判定原則
- 禁止用假設性 client（CLI、測試工具等）或假設性威脅作為 fallback 必要性論證，除非系統實際存在這些 client
- 修正 AI 容易把死代碼養成必要代碼的問題：判定流程改為先看正規使用路徑是否可達，不可達直接視為死代碼
