...
本 reference 的輸出會進入 Step 4 的 Sonnet 統包 agent。該 agent 依本 agent 回報結果，套用以下三擇一狀態判定（此規則以本檔為唯一真相來源，SKILL.md 不重複定義細節）：

- **資訊不足**：本 agent 回報「資訊不足，跳過」→ 狀態欄填「ℹ️ 資訊不足」，不產生任何修正條目
- **命中率 ≥ 60%**：狀態欄填「✅ OK」，不產生修正條目（命名已與需求相符）
- **命中率 < 60%**：狀態欄填「⚠️ 建議調整」，「建議調整命名」逐條進入待驗證修正清單

任何調整這三個分支的行為，都應該在本檔修改，不要改 SKILL.md。
