...
```bash
python3 scripts/slack-chat.py channels
```
...
```bash
python3 scripts/slack-chat.py send --channel "#頻道名稱" --message "訊息內容"
```
...
```bash
python3 scripts/slack-chat.py send --channel "#頻道名稱" --message "fallback 純文字" --blocks-json /tmp/blocks.json
```
...
```bash
python3 scripts/slack-chat.py upload --channel "#頻道" --file /path/to/file --message "附加說明（可選）"
```
...
- `invalid_auth` → Token 無效，請使用者檢查 `.env`
