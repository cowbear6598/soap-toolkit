...
- 特別提醒：計畫書中若需出現 fallback / defensive code（try-catch、null/undefined fallback、預設值兜底、retry、型別系統已保證的防衛），必須遵守 `plan-guide.md` 的「Fallback / Defensive Code 防護規則」，先用 AskUserQuestion 與使用者對齊每一個的必要性後才能寫入 todo
