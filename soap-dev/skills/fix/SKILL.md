...
## Step 4：為每個修復項目啟動 agent 執行修復

### 判斷是否可並行

- 檢查各修復項目是否有**明顯的共同檔案修改衝突**
- **「明顯衝突」判定標準**：兩個修復項目是否**宣告修改同一檔案**。若有任一檔案被兩個以上項目同時宣告要改 → 視為衝突
- **有衝突** → 依序執行，一個做完再做下一個
- **無衝突** → 可並行，同時啟動多個 agent

### 啟動 agent

每個修復項目啟動一個獨立 agent（使用 Task tool，**不指定 subagent_type**，`model: sonnet`）。

### 嚴格遵守的原則

- **一個 agent 只負責一個修復項目**，絕對不要把多個項目塞進同一個 agent
- **不要假設 agent 知道任何背景**，agent 是全新的、沒讀過報告、沒看過 codebase，所有資訊都要寫進 prompt

### 傳給 agent 的 prompt 必須包含以下三個部分

1. **該修復項目的完整描述**：檔案路徑（含行號若有）、要做的改動（行為與規格描述）、驗收條件或問題來源。要具體到檔案與要做的改動，不得只丟模糊描述。
