---
name: review
description: "對程式碼進行全面 review，涵蓋複雜度、品質、安全性、風格、測試覆蓋、冗餘程式碼、效能、AI 可讀性八大面向。當使用者提到 review、code review、檢查程式碼、PR review、或想確認程式碼品質時觸發。即使使用者只說「幫我看一下」或「檢查一下有沒有問題」，只要是在談論程式碼品質，就應該使用這個 skill。觸發後若 git 工作樹沒有任何改動，主動詢問使用者要 review 哪些檔案、PR 網址、或具體程式碼片段再進行；若使用者想在動手前討論方向而非檢查已寫好的 code，請改用 discuss 或 plan。"
...
Sonnet agent 的回報只有兩種結論：「屬實」或「不屬實」，不設信心度分數、不設「不確定」選項。若回報「不屬實」，必須附上具體理由（事實依據來自實際打開的檔案內容）。

**主 agent 在此階段不做任何抽驗或二次判斷**：Sonnet 回報屬實 → 條目進修正表格；Sonnet 回報不屬實 → 條目進「✅ 排除誤判」區塊。完全信任 Sonnet 的回報結果。
...
3. AI 可讀性三擇一狀態判定規則請見 references/ai-readability.md 的「與統包 agent 的介面契約」節。依該契約填入狀態欄與決定是否產出修正條目。
...
- 若檔案不存在、行號越界、或檔案為二進位無法解析，判為「不屬實」，排除理由寫「檔案/行號已失效：<具體原因>」。不需要新增第三類別，這屬於「修正方向錯誤」的延伸 — 因為連原程式碼位置都無法定位，修正方向已無意義。
